...
if (Test-Path -LiteralPath $bundledPython) {
    ...
    throw "No usable Python runtime was found for the IARIC continuation"
}

$outputDirectory = Join-Path $RepositoryRoot "backtests\output\stock\iaric\round_4\phased_auto_representative_v3"
$stdoutPath = Join-Path $outputDirectory "continuation_background_stdout.log"
$stderrPath = Join-Path $outputDirectory "continuation_background_stderr.log"
$supervisorStatusPath = Join-Path $outputDirectory "continuation_supervisor_status.json"
$pollSeconds = 30
...
for ($attempt = 1; $attempt -le $MaxRestarts; $attempt++) {
    $attemptStdout = Join-Path $outputDirectory "continuation_attempt_${attempt}_stdout.log"
    $attemptStderr = Join-Path $outputDirectory "continuation_attempt_${attempt}_stderr.log"
    $arguments = @(
        "-m",
        "backtests.stock.auto.runners.run_iaric_round4_continuation",
        "--output-dir",
        $outputDirectory,
        "--max-workers",
        "2"
    ...
    while (-not $runner.HasExited) {
        ...
        Write-JsonStatus -Path $supervisorStatusPath -Payload @{
            status = "monitoring_round4_continuation"
            attempt = $attempt
    ...
    if ($exitCode -in @(0, 2)) {
        Write-JsonStatus -Path $supervisorStatusPath -Payload @{
            status = if ($exitCode -eq 0) { "complete_promoted" } else { "complete_gates_blocked" }
            attempt = $attempt
            exit_code = $exitCode
