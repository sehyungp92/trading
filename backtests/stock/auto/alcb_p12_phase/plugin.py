...
PHASE_STATIC_HARD_REJECTS: dict[int, dict[str, float]] = {
    1: {
        ...
        "min_pf": 1.10,
        "min_entry_quality": 0.40,
        "max_dd_pct": 0.08,
    },
    2: {
        "min_expected_total_r": 5.5,
    ...
    3: {
        ...
        "min_profit_protection": 0.25,
        "max_dd_pct": 0.07,
    },
    4: {
        "min_expected_total_r": 6.5,
        "min_trades_per_month": 14.0,
        "min_pf": 1.18,
        "min_inv_dd": 0.40,
        "max_dd_pct": 0.06,
    ...
    5: {
        "min_expected_total_r": 7.0,
        "min_trades_per_month": 12.0,
        "min_pf": 1.20,
        "min_inv_dd": 0.42,
        "min_entry_quality": 0.50,
        "max_dd_pct": 0.06,
    },
}
...
ULTIMATE_TARGETS = {
    "expectancy_dollar": 2.50,
    "expected_total_r": 20.0,
    "trades_per_month": 25.0,
    "profit_factor": 1.60,
    "entry_quality": 0.70,
    "high_rvol_edge": 0.70,
    "short_hold_drag_inverse": 0.60,
    "flow_reversal_short_inverse": 0.45,
    "early_1000_drag_inverse": 0.50,
    "profit_protection": 0.55,
...
class ALCBP12Plugin:
    ...
    def build_end_of_round_artifacts(self, state: PhaseState) -> EndOfRoundArtifacts:
        ...
        overall_verdict = (
            f"Final 5-phase bundle finished at "
    ...
    def get_diagnostic_gaps(self, phase: int, metrics: dict[str, float]) -> list[str]:
        ...
        if phase == 1:
            ...
            if metrics["trades_per_month"] < max(15.0, base_metrics.get("trades_per_month", 0.0) * 0.50):
            ...
            if metrics["entry_quality"] < max(0.55, base_metrics.get("entry_quality", 0.0) * 0.98):
    ...
    def _gate_criteria(self, phase: int, metrics: dict[str, float]) -> list[GateCriterion]:
        ...
        if phase == 1:
            return [
                ...
                self._min_gate("trades_per_month", metrics["trades_per_month"], max(15.0, base_metrics.get("trades_per_month", 0.0) * 0.50)),
        ...
        if phase == 2:
            return [
                ...
                self._min_gate("expectancy_dollar", metrics["expectancy_dollar"], max(0.50, base_metrics.get("expectancy_dollar", 0.0) * 0.98)),
                self._min_gate("trades_per_month", metrics["trades_per_month"], max(14.0, base_metrics.get("trades_per_month", 0.0) * 0.80)),
                self._max_gate("max_drawdown_pct", metrics["max_drawdown_pct"], dd_budget),
            ]
        if phase == 3:
            return [
                ...
                self._min_gate("trades_per_month", metrics["trades_per_month"], max(14.0, base_metrics.get("trades_per_month", 0.0) * 0.85)),
                self._max_gate("max_drawdown_pct", metrics["max_drawdown_pct"], dd_budget),
            ]
        if phase == 4:
            return [
                self._min_gate("expectancy_dollar", metrics["expectancy_dollar"], max(0.70, base_metrics.get("expectancy_dollar", 0.0) * 1.0)),
                self._min_gate("expected_total_r", metrics["expected_total_r"], max(6.5, base_metrics.get("expected_total_r", 0.0) * 1.0)),
                self._min_gate("profit_factor", metrics["profit_factor"], max(1.18, base_metrics.get("profit_factor", 0.0) * 1.0)),
                self._min_gate("inv_dd", metrics["inv_dd"], max(0.42, base_metrics.get("inv_dd", 0.0) * 0.98)),
                self._min_gate("trades_per_month", metrics["trades_per_month"], max(14.0, base_metrics.get("trades_per_month", 0.0) * 0.90)),
                self._max_gate("max_drawdown_pct", metrics["max_drawdown_pct"], dd_budget),
            ]
        # phase == 5
        return [
            self._min_gate("expectancy_dollar", metrics["expectancy_dollar"], max(0.80, base_metrics.get("expectancy_dollar", 0.0) * 1.0)),
            self._min_gate("expected_total_r", metrics["expected_total_r"], max(7.0, base_metrics.get("expected_total_r", 0.0) * 1.0)),
            self._min_gate("profit_factor", metrics["profit_factor"], max(1.20, base_metrics.get("profit_factor", 0.0) * 1.0)),
            self._min_gate("inv_dd", metrics["inv_dd"], max(0.45, base_metrics.get("inv_dd", 0.0) * 0.98)),
            self._min_gate("trades_per_month", metrics["trades_per_month"], max(12.0, base_metrics.get("trades_per_month", 0.0) * 0.90)),
            self._min_gate("entry_quality", metrics["entry_quality"], max(0.50, base_metrics.get("entry_quality", 0.0) * 0.98)),
            self._max_gate("max_drawdown_pct", metrics["max_drawdown_pct"], dd_budget),
        ]
    ...
    def _resolve_phase_hard_rejects(
        ...
        if phase == 1:
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.90, minimum=5.0)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.50, minimum=15.0)
            self._raise_floor(resolved, "min_entry_quality", base_metrics.get("entry_quality", 0.0), 0.95, minimum=0.40)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.98, minimum=1.10)
        elif phase == 2:
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.90, minimum=5.5)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.80, minimum=14.0)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.97, minimum=1.12)
            self._raise_floor(resolved, "min_profit_protection", base_metrics.get("profit_protection", 0.0), 0.98, minimum=0.20)
        elif phase == 3:
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.92, minimum=6.0)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.85, minimum=14.0)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.97, minimum=1.15)
            self._raise_floor(resolved, "min_profit_protection", base_metrics.get("profit_protection", 0.0), 0.98, minimum=0.25)
            self._raise_floor(resolved, "min_long_hold_capture", base_metrics.get("long_hold_capture", 0.0), 0.95, minimum=0.35)
        elif phase == 4:
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.95, minimum=6.5)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.88, minimum=14.0)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.98, minimum=1.18)
            self._raise_floor(resolved, "min_inv_dd", base_metrics.get("inv_dd", 0.0), 0.95, minimum=0.40)
        else:  # phase 5
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.98, minimum=7.0)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.90, minimum=12.0)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.99, minimum=1.20)
            self._raise_floor(resolved, "min_inv_dd", base_metrics.get("inv_dd", 0.0), 0.95, minimum=0.42)
            self._raise_floor(resolved, "min_entry_quality", base_metrics.get("entry_quality", 0.0), 0.98, minimum=0.50)
    ...
    def _drawdown_budget(phase: int, base_metrics: dict[str, float]) -> float:
        ...
        floor = 0.06 if phase >= 4 else 0.07
        dynamic = max(base_dd * multiplier, base_dd + 0.015, floor)
        return min(0.08, dynamic)
...
def _build_phase_snapshot(
    ...
    *,
    ...
    return "\n".join([
        "=" * 70,
        f"ALCB P12 SHARED PHASE {phase} SNAPSHOT",
        "=" * 70,
        f"Focus: {focus}",
