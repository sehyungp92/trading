...
def set_phase_config(args: tuple) -> None:
    ...
    Called via ``pool.map`` at the start of each phase so that
    ``score_candidate`` can use 3-tuple args (name, mutations,
    current_mutations) without per-candidate pickling of phase config.
    """
...
# Hard reject checks available from base PerformanceMetrics without trade-level
# analysis.  These mirror the corresponding entries in phase_reject_reason but
# can be evaluated *before* the expensive compute_alcb_phase_metrics call.
...
def _early_reject_reason(perf, hard_rejects: dict | None, *, phase: int = 0) -> str:
    ...
    ``expected_total_r`` and ``inv_dd`` are computed identically to
    ``enrich_alcb_phase_metrics`` but only require fields already present
    on the PerformanceMetrics dataclass, avoiding the O(trades) work in
    ...
    # Read fields directly from the dataclass (avoids full asdict copy).
    ...
    # Cheaply derived enriched fields (same formulas as enrich_alcb_phase_metrics).
    values = {
