...
# P13 uses much higher expected_total_r floors than P12 because the fixed
# normalization ceilings make the scorer sensitive to real R degradation.
# ---------------------------------------------------------------------------
PHASE_STATIC_HARD_REJECTS: dict[int, dict[str, float]] = {
    ...
    3: {
        ...
        "min_trades_per_month": 12.0,
        "min_pf": 1.18,
        "min_long_hold_capture": 0.30,
        "max_dd_pct": 0.07,
    },
    4: {
        "min_expected_total_r": 45.0,
        "min_trades_per_month": 12.0,
        "min_pf": 1.20,
        "min_inv_dd": 0.35,
        "max_dd_pct": 0.065,
...
PHASE_MAX_ROUNDS = {1: 14, 2: 12, 3: 10, 4: 12, 5: 14}
PHASE_PRUNE_THRESHOLDS = {1: 0.05, 2: 0.04, 3: 0.04, 4: 0.03, 5: 0.03}
PHASE_MIN_EFFECTIVE_DELTA = {1: 0.003, 2: 0.003, 3: 0.002, 4: 0.002, 5: 0.002}

ULTIMATE_TARGETS = {
    "expectancy_dollar": 6.00,
...
class ALCBP13Plugin:
    ...
    def get_diagnostic_gaps(self, phase: int, metrics: dict[str, float]) -> list[str]:
        ...
        if phase == 1:
            if metrics["entry_quality"] < max(0.50, base_metrics.get("entry_quality", 0.0) * 1.02):
                gaps.append("Entry-quality mix is still too exposed to weak RVOL and COMBINED cohorts.")
            if metrics["high_rvol_edge"] < max(0.45, base_metrics.get("high_rvol_edge", 0.0) * 1.02):
                gaps.append("High-RVOL entry cohort edge remains weak -- RVOL floor or COMBINED kill insufficient.")
            if metrics["trades_per_month"] < max(12.0, base_metrics.get("trades_per_month", 0.0) * 0.50):
                gaps.append("Filtering is cutting too much throughput.")
        elif phase == 2:
            if metrics["profit_protection"] < max(0.30, base_metrics.get("profit_protection", 0.0) * 1.03):
                gaps.append("Short-hold drag reforms have not sufficiently reduced purgatory-zone losses.")
            if metrics["short_hold_drag_inverse"] < max(0.35, base_metrics.get("short_hold_drag_inverse", 0.0) * 1.03):
                gaps.append("Short-hold losers are still consuming too much total R.")
        elif phase == 3:
            if metrics["long_hold_capture"] < max(0.50, base_metrics.get("long_hold_capture", 0.0) * 1.0):
                gaps.append("Winner protection is not preserving enough of the long-hold alpha.")
            if metrics["profit_protection"] < max(0.30, base_metrics.get("profit_protection", 0.0) * 0.98):
                gaps.append("Trail tuning is eroding profit protection from Phase 2.")
        elif phase == 4:
            ...
            if metrics["inv_dd"] < max(0.45, base_metrics.get("inv_dd", 0.0) * 0.98):
                gaps.append("Risk adjustments are pushing drawdown above acceptable bounds.")
            if metrics["trades_per_month"] < max(12.0, base_metrics.get("trades_per_month", 0.0) * 0.90):
                gaps.append("Timing changes are cutting throughput too aggressively.")
        elif phase == 5:
            if metrics["long_hold_capture"] < max(0.45, base_metrics.get("long_hold_capture", 0.0) * 0.98):
                gaps.append("Winner capture mechanisms are not improving long-hold alpha retention.")
            if metrics["expectancy_dollar"] < base_metrics.get("expectancy_dollar", 0.0) * 0.95:
                gaps.append("Capture experiments are eroding per-trade expectancy.")
            if metrics["expected_total_r"] < base_metrics.get("expected_total_r", 0.0) * 0.90:
                gaps.append("Total R is declining -- capture changes may be cutting winners short.")
        return gaps
    ...
    def _gate_criteria(self, phase: int, metrics: dict[str, float]) -> list[GateCriterion]:
        ...
        if phase == 1:
            return [
                ...
                self._min_gate("trades_per_month", metrics["trades_per_month"], max(12.0, base_metrics.get("trades_per_month", 0.0) * 0.50)),
                self._min_gate("profit_factor", metrics["profit_factor"], max(1.10, base_metrics.get("profit_factor", 0.0) * 1.0)),
                self._min_gate("expected_total_r", metrics["expected_total_r"], max(30.0, base_metrics.get("expected_total_r", 0.0) * 0.90)),
        ...
        if phase == 2:
            return [
                ...
                self._min_gate("short_hold_drag_inverse", metrics["short_hold_drag_inverse"], max(0.30, base_metrics.get("short_hold_drag_inverse", 0.0) * 1.05)),
                self._min_gate("expectancy_dollar", metrics["expectancy_dollar"], max(1.00, base_metrics.get("expectancy_dollar", 0.0) * 0.98)),
                self._min_gate("trades_per_month", metrics["trades_per_month"], max(12.0, base_metrics.get("trades_per_month", 0.0) * 0.80)),
                self._max_gate("max_drawdown_pct", metrics["max_drawdown_pct"], dd_budget),
            ]
        if phase == 3:
            return [
                self._min_gate("long_hold_capture", metrics["long_hold_capture"], max(0.45, base_metrics.get("long_hold_capture", 0.0) * 1.03)),
                self._min_gate("profit_protection", metrics["profit_protection"], max(0.28, base_metrics.get("profit_protection", 0.0) * 0.98)),
                self._min_gate("expectancy_dollar", metrics["expectancy_dollar"], max(1.50, base_metrics.get("expectancy_dollar", 0.0) * 1.0)),
                self._min_gate("expected_total_r", metrics["expected_total_r"], max(35.0, base_metrics.get("expected_total_r", 0.0) * 0.95)),
                self._min_gate("trades_per_month", metrics["trades_per_month"], max(12.0, base_metrics.get("trades_per_month", 0.0) * 0.85)),
                self._max_gate("max_drawdown_pct", metrics["max_drawdown_pct"], dd_budget),
            ]
        if phase == 4:
            return [
                self._min_gate("expectancy_dollar", metrics["expectancy_dollar"], max(2.00, base_metrics.get("expectancy_dollar", 0.0) * 1.0)),
                self._min_gate("expected_total_r", metrics["expected_total_r"], max(40.0, base_metrics.get("expected_total_r", 0.0) * 0.98)),
                self._min_gate("profit_factor", metrics["profit_factor"], max(1.20, base_metrics.get("profit_factor", 0.0) * 1.0)),
                self._min_gate("inv_dd", metrics["inv_dd"], max(0.42, base_metrics.get("inv_dd", 0.0) * 0.98)),
                self._min_gate("trades_per_month", metrics["trades_per_month"], max(12.0, base_metrics.get("trades_per_month", 0.0) * 0.90)),
                self._max_gate("max_drawdown_pct", metrics["max_drawdown_pct"], dd_budget),
            ]
        # phase == 5: permissive gates focused on capture without destroying alpha
        return [
            self._min_gate("long_hold_capture", metrics["long_hold_capture"], max(0.40, base_metrics.get("long_hold_capture", 0.0) * 0.95)),
            self._min_gate("expectancy_dollar", metrics["expectancy_dollar"], max(1.50, base_metrics.get("expectancy_dollar", 0.0) * 0.95)),
            self._min_gate("expected_total_r", metrics["expected_total_r"], max(30.0, base_metrics.get("expected_total_r", 0.0) * 0.90)),
            self._min_gate("profit_factor", metrics["profit_factor"], max(1.15, base_metrics.get("profit_factor", 0.0) * 0.95)),
            self._min_gate("trades_per_month", metrics["trades_per_month"], max(10.0, base_metrics.get("trades_per_month", 0.0) * 0.85)),
            self._max_gate("max_drawdown_pct", metrics["max_drawdown_pct"], dd_budget),
        ]
    ...
    def _resolve_phase_hard_rejects(
        ...
        if phase == 1:
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.55, minimum=30.0)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.40, minimum=12.0)
            self._raise_floor(resolved, "min_entry_quality", base_metrics.get("entry_quality", 0.0), 0.90, minimum=0.35)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.80, minimum=1.10)
        elif phase == 2:
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.60, minimum=35.0)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.50, minimum=12.0)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.85, minimum=1.15)
            self._raise_floor(resolved, "min_profit_protection", base_metrics.get("profit_protection", 0.0), 0.90, minimum=0.20)
        elif phase == 3:
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.70, minimum=40.0)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.55, minimum=12.0)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.88, minimum=1.18)
            self._raise_floor(resolved, "min_long_hold_capture", base_metrics.get("long_hold_capture", 0.0), 0.90, minimum=0.30)
        elif phase == 4:
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.80, minimum=45.0)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.60, minimum=12.0)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.90, minimum=1.20)
            self._raise_floor(resolved, "min_inv_dd", base_metrics.get("inv_dd", 0.0), 0.90, minimum=0.35)
        else:  # phase 5: permissive floors -- let experiments through
            self._raise_floor(resolved, "min_expected_total_r", base_metrics.get("expected_total_r", 0.0), 0.55, minimum=30.0)
            self._raise_floor(resolved, "min_trades_per_month", base_metrics.get("trades_per_month", 0.0), 0.45, minimum=10.0)
            self._raise_floor(resolved, "min_pf", base_metrics.get("profit_factor", 0.0), 0.78, minimum=1.20)

        resolved["max_dd_pct"] = min(
            float(resolved.get("max_dd_pct", 0.08)),
    ...
    def _drawdown_budget(phase: int, base_metrics: dict[str, float]) -> float:
        ...
        static_cap = {1: 0.08, 2: 0.075, 3: 0.07, 4: 0.065, 5: 0.06}.get(phase, 0.075)
        floor = 0.065 if phase == 4 else (0.06 if phase == 5 else 0.07)
...
def _build_phase_snapshot(
    ...
    *,
    ...
    return "\n".join([
        "=" * 70,
        f"ALCB P13 SHARED PHASE {phase} SNAPSHOT",
        "=" * 70,
        f"Focus: {focus}",
