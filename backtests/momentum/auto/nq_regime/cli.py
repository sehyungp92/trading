"""NQ_REGIME phased auto-optimization CLI."""
...
ROUND_MANAGER = RoundManager("momentum", "nq_regime")
PHASE_CHOICES = list(range(1, NqRegimePlugin.num_phases + 1))
...
def _build_runner(args: argparse.Namespace, *, for_write: bool = True) -> PhaseRunner:
    plugin = NqRegimePlugin(
        ...
        num_phases=NqRegimePlugin.num_phases,
    )
    ...
    if output_dir:
        ...
        return PhaseRunner(
            ...
            round_name=getattr(args, "round_name", "") or Path(output_dir).name,
...
def cmd_phase_auto(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "output_dir", None):
        _write_custom_round_artifacts(args, runner, state)
    print("NQ_REGIME auto-optimization complete.")
...
def _write_custom_round_artifacts(args: argparse.Namespace, runner: PhaseRunner, state) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="nq-regime-auto", description="NQ_REGIME phased auto-optimization")
    ...
    def add_common(cmd: argparse.ArgumentParser) -> None:
        ...
        cmd.add_argument("--round-name", default="")

    phase_run = sub.add_parser("phase-run", help="Run one NQ_REGIME phase")
    ...
    phase_auto = sub.add_parser("phase-auto", help="Run all NQ_REGIME phases")
