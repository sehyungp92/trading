...
from backtest.config_downturn import DownturnAblationFlags
...
# ---------------------------------------------------------------------------
# Reversal engine signal detection  (Spec §4)
# ---------------------------------------------------------------------------
...
def detect_reversal_short(
    ...
    Gate: 2-of-3 weakening trend / extended above mean / vol coil  (Spec §4.1)
...
# ---------------------------------------------------------------------------
# Breakdown engine signal detection  (Spec §6)
# ---------------------------------------------------------------------------
...
def detect_breakdown_short(
    ...
    Structural: close < box_low  (Spec §6.2)
    Displacement: metric >= rolling quantile  (Spec §6.2)
    Spike reject: excessive range with small body  (Spec §6.2)
    """
    po = param_overrides or {}
    ...
    # Spike reject: range > 1.8×ATR AND body_ratio < 0.30 AND RVOL > 2.0
...
# ---------------------------------------------------------------------------
# Fade engine signal detection  (Spec §7)
# ---------------------------------------------------------------------------
...
def detect_fade_short(
    ...
    Touch: high >= VWAP within last 8×15m bars  (Spec §7.2)
    Rejection: close < VWAP  (Spec §7.2)
    Cap gate: close >= VWAP - b_cap × ATR15  (Spec §7.2)
    Momentum: SlopeOK_short(t)  (Spec §7.4)
    """
    po = param_overrides or {}
    ...
    # Cap gate: close >= VWAP - b_cap × ATR15
...
# ---------------------------------------------------------------------------
# Entry subtype + stop computation  (Spec §10)
# ---------------------------------------------------------------------------
