"""Breakout walk-forward validation with expanding training windows.

Mirrors HelixWalkForwardValidator but uses Breakout types.
...
class BreakoutWalkForwardValidator:
    """Expanding-window walk-forward validation for Breakout v3.3-ETF."""
    ...
    def _run_fold(self, fold: WalkForwardFold) -> WalkForwardFold:
        ...
        train_config = BreakoutBacktestConfig(
        ...
        optimizer = BreakoutOptimizationRunner(
        ...
        test_config = BreakoutBacktestConfig(
            symbols=self.base_config.symbols,
            initial_equity=self.base_config.initial_equity,
            slippage=self.base_config.slippage,
            flags=self.base_config.flags,
        ...
        # Use synchronized mode for test evaluation (realistic portfolio constraints)
        test_result = run_breakout_synchronized(test_data, test_config)
    ...
    def _slice_data(self, start, end) -> BreakoutPortfolioData:
        """Slice BreakoutPortfolioData to a time range with 4H recomputation."""
        sliced = BreakoutPortfolioData()
