"""Round 7.2: Full 4-phase greedy optimization with R7.2 scoring rebalance.

Re-runs all 4 phases from R6 baseline using the updated scoring system:
  - Detection weight: 7% → 18%
  - 3-part detection: bias_latency + trade_latency + coverage
  - Normalization: bias/20d, trade/30d (no more cliff at 15d)

Uses the same phase structure as original rounds 3-6 but with the
rebalanced scoring, which should unlock regime detection improvements
that were previously invisible to the optimizer.

Usage:
    python research/backtests/swing/auto/brs/run_r72_full_phased.py
"""
...
# R6 baseline mutations (locked — starting point for all phases)
R6_BASELINE = {
    "disable_s1": False,
...
def main():
    start = time.time()
    run_id = f"r72_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"

    print("=" * 70)
    print("BRS R7.2 FULL PHASED OPTIMIZATION")
    print(f"Run ID: {run_id}")
    print(f"Scoring: R7.2 rebalanced (detection 18%, bias_latency metric)")
    print(f"Baseline: R6 cumulative mutations ({len(R6_BASELINE)} params)")
    print("=" * 70)

    # Backup current phase_state.json
    backup_path = OUTPUT_DIR / f"phase_state_pre_r72_backup.json"
    ...
    # Initialize fresh phase state from R6 baseline
    state = PhaseState(
        current_phase=0,
        completed_phases=[],
        cumulative_mutations=dict(R6_BASELINE),
    )

    cumulative = dict(R6_BASELINE)
    phase_logs = []
    ...
    for phase in range(1, NUM_PHASES + 1):
        ...
        # Save greedy result
        greedy_path = OUTPUT_DIR / f"r72_phase{phase}_greedy.json"
    ...
    print(f"\n{'='*70}")
    print("R7.2 FULL PHASED OPTIMIZATION COMPLETE")
    print(f"{'='*70}")
    ...
    for k, v in sorted(cumulative.items()):
        changed = k not in R6_BASELINE or R6_BASELINE[k] != v
        marker = " [NEW]" if k not in R6_BASELINE else (" [CHANGED]" if changed else "")
        print(f"  {k}: {v}{marker}")

    new_mutations = {k: v for k, v in cumulative.items() if k not in R6_BASELINE or R6_BASELINE[k] != v}
    print(f"\nNew/changed mutations: {len(new_mutations)}")
    for k, v in sorted(new_mutations.items()):
        old = R6_BASELINE.get(k, "(new)")
        print(f"  {k}: {old} -> {v}")

    # Run final comparison
    print("\nComputing R6 baseline vs R7.2 final metrics...")
    r6_metrics = run_backtest_metrics(R6_BASELINE)
    r72_metrics = run_backtest_metrics(cumulative)
    ...
    print(f"\n{'Metric':<30} {'R6':>12} {'R7.2':>12} {'Delta':>12}")
    print("-" * 68)
    for label, key, thresh, direction in comparison_keys:
        r6v = r6_metrics.get(key, 0)
        r72v = r72_metrics.get(key, 0)
        if isinstance(r6v, int):
            print(f"{label:<30} {r6v:>12d} {r72v:>12d} {r72v - r6v:>+12d}")
        else:
            print(f"{label:<30} {r6v:>12.3f} {r72v:>12.3f} {r72v - r6v:>+12.3f}")

    # Save full report
    report_path = OUTPUT_DIR / f"r72_full_diagnostics.txt"
    with open(report_path, "w") as f:
        f.write(f"BRS R7.2 Full Phased Optimization\n")
        f.write(f"Run ID: {run_id}\n")
        f.write(f"Elapsed: {total_elapsed:.0f}s\n")
        f.write(f"Scoring: R7.2 rebalanced (detection 18%, bias_latency)\n\n")
        for log in phase_logs:
            f.write(log + "\n")
        f.write(f"\nCumulative mutations: {json.dumps(cumulative, indent=2)}\n")
        f.write(f"\nR6 metrics: {json.dumps(r6_metrics, indent=2)}\n")
