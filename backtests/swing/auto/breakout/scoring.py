...
from backtests.swing.auto.scoring import CompositeScore, composite_score

PHASE_WEIGHTS: dict[int, dict[str, float]] = {
    1: {"net_profit": 0.25, "pf": 0.35, "calmar": 0.10, "inv_dd": 0.30},
    2: {"net_profit": 0.30, "pf": 0.25, "calmar": 0.15, "inv_dd": 0.30},
    3: {"net_profit": 0.25, "pf": 0.25, "calmar": 0.20, "inv_dd": 0.30},
}

PHASE_HARD_REJECTS: dict[int, dict[str, float]] = {
    1: {"min_trades": 20, "max_dd_pct": 0.20, "min_pf": 1.00},
    2: {"min_trades": 20, "max_dd_pct": 0.18, "min_pf": 1.05},
    3: {"min_trades": 25, "max_dd_pct": 0.15, "min_pf": 1.10},
}
    ...
    *,
    ...
    if metrics.total_trades < min_trades:
        return CompositeScore(
            0, 0, 0, 0, 0,
            rejected=True,
            reject_reason=f"phase{phase}_too_few_trades ({metrics.total_trades} < {min_trades})",
        )
    ...
    if max_dd_pct is not None and metrics.max_drawdown_pct > float(max_dd_pct):
        return CompositeScore(
            ...
            reject_reason=(
                ...
                f"({metrics.max_drawdown_pct:.1%} > {float(max_dd_pct):.1%})"
            ),
        )
    ...
    if min_pf is not None and metrics.profit_factor < float(min_pf):
        return CompositeScore(
            0, 0, 0, 0, 0,
            rejected=True,
            reject_reason=f"phase{phase}_low_pf ({metrics.profit_factor:.2f} < {float(min_pf):.2f})",
        )
