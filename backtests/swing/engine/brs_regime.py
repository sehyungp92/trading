...
def classify_regime(
    ...
    Args:
        ...
        ema_fast: EMA(20) value
        ema_slow: EMA(50) value
        regime_on: Whether regime is active (ADX hysteresis)
        adx_strong: ADX threshold for BEAR_STRONG (default 28)
        bear_min_conditions: Min bear conditions for BEAR_FORMING (4=disabled)

    Returns:
        BRSRegime enum value
    """
    if not regime_on:
    ...
    # BEAR_STRONG: ADX strong + ema_fast < ema_slow + -DI > +DI (unchanged)
    if adx >= adx_strong and ema_fast < ema_slow and minus_di > plus_di:
        return BRSRegime.BEAR_STRONG

    # BEAR_TREND: all 3 conditions (unchanged)
    if bear_conds == 3:
        return BRSRegime.BEAR_TREND

    # BEAR_FORMING: partial conditions met (only when enabled via config)
    # bear_conds < 3 is guaranteed here since bear_conds == 3 returns BEAR_TREND above
    if bear_min_conditions < 4 and bear_conds >= bear_min_conditions:
        return BRSRegime.BEAR_FORMING

    # BULL_TREND (unchanged)
...
def update_bias(
    ...
    # Path F: return-only bias confirmation (Round 6 Part B)
    ...
    Bear confirmation (8 paths):
      Path A (standard): regime_on + hold_count >= 2 (or reduced via 4H accel)
      Path B (fast-crash): regime_on + hold_count >= 1 + bear_score >= 50 + ADX >= 20
      Path C (structure): regime_on + hold_count >= 1 + di_diff >= 8 + ema_sep >= 0.15 + ADX >= 18
      Path D (vol spike): regime_on + hold_count >= 1 + extreme daily drop + ATR expansion
      Path E (crash override): NO regime_on needed + hold >= 1 + daily drop > 3% + vol expanding
      Path F (return-only): NO regime_on needed + hold >= 1 + daily return < thresh + close < ema_fast
      Path G (cumulative return): NO regime_on needed + hold >= 1 + N-day cum return < thresh + close < ema_fast

    Bull confirmation (strict):
      hold_count >= 3 (one extra day vs bear)
    """
    new_bias = BiasState(
    ...
    if raw_direction == prev_bias.raw_direction and raw_direction != Direction.FLAT:
        ...
        # Check churn bridge: if returning to SHORT after brief FLAT
        if (raw_direction == Direction.SHORT
            ...
            # Restore hold_count instead of resetting to 1
        ...
        # FLAT: track streak for churn bridge
    ...
    # Path E: Crash override — bypasses regime_on for extreme conditions.
    # This fires FIRST, before regime_on checks, to catch sudden crashes
    # (COVID, SVB) where ADX lags behind price action.
    ...
    # Path F: Return-only bias confirmation — bypasses regime_on.
    # Catches SVB-type drops where -1.73% daily return is too mild for
    # crash override (-3%) but clearly bearish when close < ema_fast.
    ...
    # Path G: Cumulative return — bypasses regime_on.
    # Catches gradual selloffs (Tariff Shock) where individual daily drops
    # are too mild for Path E/F but N-day cumulative return is clearly bearish.
    ...
    # Carry forward crash_override if still SHORT (decays when raw flips)
    ...
    if raw_direction == Direction.SHORT and regime_on:
        # Path A: standard hold_count check (default 2-day, reduced by 4H accel)
        hold_req = 2
        ...
        # Path D: fast-crash (extreme day) — uses configurable thresholds
            ...
            # Keep previous confirmed if it was SHORT
        ...
        # If crash_override is active, keep SHORT confirmed
