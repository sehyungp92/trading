...
def check_s1(
    ...
    Requires:
      - Daily confirmed BEAR bias
      - regime_on
      - 4H regime BEAR or CHOP
      - Hourly: touch EMA_pull, reject below, momentum filter, micro confirm
    """
    if cfg.disable_s1:
        return None
    ...
    # EMA_pull is regime-adaptive
    ema_pull = h.ema_pull
    if ema_pull <= 0:
        return None

    # Short pullback entry (spec §7.2):
    # 1. Touch: high >= EMA_pull
    if h.high < ema_pull:
        return None
    # 2. Rejection: close < EMA_pull
    if h.close >= ema_pull:
        return None
    # 3. Momentum: close < EMA_mom
    if h.close >= h.ema_mom:
        return None
    # 4. Micro: close < prior_low
    if h.prior_low > 0 and h.close >= h.prior_low:
        return None

    # Compute stop — structure-first with ATR buffer
    ...
    stop_price = h.close + stop_distance  # short stop is above entry
...
def check_s2(
    ...
    Requires:
      - S2 armed (breakdown qualified)
      - Daily confirmed BEAR bias
      - BEAR_TREND or BEAR_STRONG
      - Pullback to AVWAP or EMA(34) fallback
    """
    if cfg.disable_s2:
        return None
    if not s2_arm.armed:
    ...
    # Pullback reference: AVWAP if available, else EMA_pull
    ...
    if h.high < pull_ref:
        return None
    # Rejection: close < pull_ref - reclaim_buffer
    ...
    # Edge stop from campaign box (spec §8.3)
    ...
    # Corridor cap: skip if stop too wide (spec §8.3)
...
def check_s3(
    ...
    Requires:
      - S3 armed (Donchian breakout occurred)
      - BEAR_STRONG or BEAR_FORMING regime
      - Pullback to EMA_mom
    """
    if cfg.disable_s3:
        return None
    ...
    # Structure-first stop with ATR buffer
    struct_dist = abs(h.high + 0.01 - h.close)
    stop_distance = struct_dist + sym_cfg.stop_buffer_atr * h.atr14_h
    stop_distance = max(stop_distance, sym_cfg.stop_floor_atr * h.atr14_h)
...
# ---------------------------------------------------------------------------
# LH — Lower-High Rejection (replaces S1 with structural confirmation)
# ---------------------------------------------------------------------------
...
def check_lh_rejection(
    ...
    Requires:
      - LH armed (prior swing high < previous swing high)
      - BEAR_STRONG: pass-through (no confirmed_direction check)
      - BEAR_TREND: requires confirmed_direction SHORT + volume expansion
      - EMA pullback rejection + momentum + micro (same as S1)
      - V-reversal filter: reject sharp 3-bar rallies
    """
    if cfg.disable_lh:
        return None
    ...
    # Regime gate — direction confirmation required in ALL bear regimes (Change #1)
    _crash = cfg.crash_override_enabled and d.bias.crash_override
    _forming = (cfg.forming_entry_enabled
                and d.regime == BRSRegime.BEAR_FORMING
                and d.atr_ratio > cfg.forming_vol_gate)
    ...
    if d.regime == BRSRegime.BEAR_STRONG:
        ...
        pass  # confirmed SHORT already checked in condition
    elif _crash or _forming:
        # Crash override or BEAR_FORMING with vol gate — allow entry
        if d.bias.confirmed_direction != Direction.SHORT:
            return None
    else:
        return None
    ...
    # EMA_pull touch + rejection (same mechanics as S1)
    ema_pull = h.ema_pull
    if ema_pull <= 0:
        return None
    if h.high < ema_pull:
    ...
    stop_distance = struct_dist + sym_cfg.stop_buffer_atr * h.atr14_h
    stop_distance = max(stop_distance, sym_cfg.stop_floor_atr * h.atr14_h)
    stop_price = h.close + stop_distance  # short stop above entry
    risk_per_unit = stop_distance
...
# ---------------------------------------------------------------------------
# BD — Breakdown Continuation (replaces dead S2/S3)
# ---------------------------------------------------------------------------
...
def check_bd_continuation(
    ...
    Requires:
      - BD armed (hourly broke Donchian low with volume + strong close)
      - BEAR_STRONG (or BEAR_TREND if bd_allow_bear_trend)
      - Pullback to EMA_mom: high >= ema_mom, close < ema_mom
      - Micro: close < prior_low
    """
    if cfg.disable_bd:
        return None
    ...
    # Stop: breakdown bar high + buffer, reject if too wide
...
def check_bd_arm(
    ...
    Requires:
      - BEAR_STRONG regime (or BEAR_TREND if bd_allow_bear_trend)
      - Low < Donchian low (range break)
      - Volume > bd_volume_mult * volume SMA20
      - Close in lower bd_close_quality of bar range (strong close)
    """
    allowed = set()
    if cfg.bd_allow_bear_strong:
        allowed.add(BRSRegime.BEAR_STRONG)
    ...
    # Volume confirmation (relaxed when persistence override active)
    if not persistence_override:
        if volume_sma20 <= 0 or hourly_volume < cfg.bd_volume_mult * volume_sma20:
            return None

    # Close quality: close in lower portion of bar range
...
def check_l1(
    ...
    Requires:
      - GLD only (or symbol with allow_long=True)
      - Daily confirmed BULL bias
      - regime_on
      - Long safety filter passes
      - 4H BULL or CHOP
      - Pullback to EMA_pull
    """
    if cfg.disable_l1:
        return None

    # GLD-only by default; other symbols need allow_long (flipped via experiments)
    if not sym_cfg.allow_long:
        return None

    # Quality score gate (D5 shows Q<0.75 has PF 0.65 vs 4.69 for Q>=0.75)
    if cfg.min_quality_score > 0 and d.vol.vol_factor < cfg.min_quality_score:
        return None

    # Confirmed BULL bias required
    if d.bias.confirmed_direction != Direction.LONG:
        return None
    if not d.regime_on:
        return None

    # Long safety filter (spec §4.1)
    if not cfg.disable_long_safety:
        if symbol == "GLD":
            # GLD exception: relaxed fast slope check
    ...
    if h.close <= h.ema_mom:
        return None
    # 4. Micro: close > prior_high
    if h.prior_high > 0 and h.close <= h.prior_high:
        return None

    # Structure-first stop with ATR buffer (long)
    ...
    # Long sizing penalty in bear regime (spec §10.4)
    quality = d.vol.vol_factor
...
def check_s2_arm(
    ...
    # Past-only quantile approximation — use fixed threshold scaled by config
    if disp < cfg.s2_disp_quantile:
        return None

    # Quality reject: extreme wick bar (spec §5.4)
    ...
    return S2ArmState(
        armed=True,
        armed_until_bar=current_bar_idx + 16,  # 16 RTH hours
        box_high=box_high,
        box_low=box_low,
        avwap_anchor=avwap,
    )
...
def check_s3_arm(
    ...
    Only in BEAR_STRONG/BEAR_FORMING + confirmed BEAR bias.
    """
    ...
    # Donchian low breakout: low < DonchianLow(26) AND close < EMA_mom
    if hourly_low >= donchian_low_val:
        return None
    ...
    return S3ArmState(
        armed=True,
        armed_until_bar=current_bar_idx + 12,  # 12 RTH hours
    )
