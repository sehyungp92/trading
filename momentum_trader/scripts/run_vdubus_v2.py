"""Run VdubusNQ v4.2 full diagnostics with MNQ (Micro E-mini Nasdaq 100).

Fixed 10 contracts per trade, full diagnostics, buy-and-hold comparison.
Output: backtest/output/vdubus_v2.txt
"""
...
EQUITY = 100_000.0
OUTPUT = Path("backtest/output/vdubus_v2.txt")
...
def main():
    ...
    symbol = "NQ"
    logger.info("Loading VdubusNQ data for v4.2 diagnostics...")
    ...
    try:
        ...
        logger.info(
            "Running VdubusNQ v4.2 backtest: MNQ, fixed_qty=%d, pv=$%.2f, comm=$%.2f/side",
    ...
    if not result.trades:
        print("VdubusNQ v4.2 (MNQ): No trades generated.")
        return
    ...
    # 1. Header
    sections.append(
        f"=== VdubusNQ v4.4 Full Diagnostics: MNQ (Micro E-mini Nasdaq 100) ===\n"
        f"Instrument:         MNQ\n"
        ...
        f"\n"
        f"Changes from v4.0 baseline:\n"
        f"  1. Midday dead-zone block (11:45-14:00 ET)\n"
        f"  2. Wider trailing stop (LOOKBACK=12, MULT_MIN=1.8, BASE=2.5, R_DIV=6.0, staged)\n"
        f"  3. Hourly alignment softened to 60% sizing multiplier (was hard block)\n"
        f"  4. OPEN→CORE trail tightening (0.80 factor for OPEN entries in CORE)"
    )
