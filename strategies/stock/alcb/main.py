"""Main runtime entrypoints for ALCB."""
...
from strategies.stock.instrumentation.src.strategy_data_providers import ALCBInstrumentationDataProvider
from libs.services.heartbeat import emit_heartbeat

from .artifact_store import load_candidate_artifact
...
async def run_intraday_session(
    ...
    if artifact is not None:
        ...
        try:
            active_artifact = load_candidate_artifact(session_date, settings=cfg)
    ...
    snapshot = ALCBEngine.try_load_state(active_artifact.trade_date, settings=cfg)
    ...
    async def _on_reconnect() -> None:
        ...
        logger.info("Market data subscriptions invalidated after reconnect")
    ...
    def _record_runtime_error(error_type: str, exc: Exception, *, severity: str = "medium") -> None:
        ...
        services.instrumentation_kit.log_error(
            ...
            severity=severity,
            category="dependency",
            context={"component": "strategy_alcb.main"},
            exc=exc,
        )
    ...
    try:
        if services.instrumentation is not None:
            services.instrumentation.attach_data_provider(ALCBInstrumentationDataProvider(engine))
...
async def main() -> None:
    ...
    logger.info(
        "ALCB strategy started; post-close scan %s ET, market %s-%s ET",
        cfg.post_close_scan,
        cfg.market_open,
        cfg.close_block_start,
    )
    ...
    while True:
        ...
        today = now.date()
        try:
            if now.time() >= cfg.post_close_scan and ran_selection_for != today:
                config_dir = Path(__file__).resolve().parents[1] / "config"
                services = await bootstrap_runtime(config_dir=config_dir, settings=cfg)
                try:
            ...
            logger.exception("ALCB main loop failed")
