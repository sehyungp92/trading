...
def _entry_bar_number(t: TradeRecord) -> int:
    ...
    minutes_from_open = (t.entry_time.hour * 60 + t.entry_time.minute) - 570
...
def _s23_time_of_day(trades: list[TradeRecord]) -> str:
    ...
    for lo, hi, label in buckets:
        group = [t for t in trades if lo <= t.entry_time.hour + t.entry_time.minute / 60 < hi]
        if group:
            lines.append(f"  {label}:")
            lines.append(_group_stats(group))

    # Catch any outside the expected windows
    outside = [t for t in trades if t.entry_time.hour < 10 or t.entry_time.hour >= 16]
    if outside:
