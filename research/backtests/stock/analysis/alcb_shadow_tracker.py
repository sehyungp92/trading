...
class ALCBShadowTracker:
    ...
    def funnel_report(self) -> str:
        ...
        lines = ["  Signal Funnel:", "  " + "-" * 40]
        prev = 0
        for stage in ["evaluated", "entry_signal",
                       "regime_gate", "momentum_score_gate", "rvol_filter",
                       "cpr_filter", "avwap_filter", "max_positions", "sector_limit",
                       "heat_cap", "entry_window", "entered"]:
            ...
            lines.append(f"  {stage:<20s} {count:>6}{drop}")
            prev = count
        return "\n".join(lines)
