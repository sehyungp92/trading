"""IARIC filter attribution -- per-gate KEEP/REVIEW verdicts.
...
Gates:
  regime_gate, sector_limit, position_cap, timing_blocked,
  confidence_red, sponsorship_filter, stale_rejection,
  price_invalidation, heat_cap
"""
...
class FilterStats:
    ...
    shadow_pct_reach_partial: float = 0.0
...
def compute_filter_stats(tracker: IARICShadowTracker) -> list[FilterStats]:
    ...
    for gate, shadows in sorted(by_gate.items()):
        ...
        fs.shadow_pct_reach_partial = sum(
            1 for s in simulated if s.simulated_exit == "PARTIAL_TARGET"
        ) / len(simulated)
...
def iaric_filter_attribution_report(
    ...
    lines = [
        ...
        "=" * 70,
        "",
        f"  {'Gate':<22s} {'Blocked':>7} {'Sim':>5} {'WR%':>6} {'Avg R':>7}"
        f" {'Part%':>6} {'Missed':>8} {'Avoided':>8} {'Verdict':>8}",
        "  " + "-" * 70,
    ]
    ...
    for fs in stats:
        ...
        lines.append(
            f"  {fs.gate:<22s} {fs.blocked_count:>7} {fs.simulated_count:>5}"
            f" {fs.shadow_win_pct:>5.0%} {fs.shadow_avg_r:>7.2f}"
            f" {fs.shadow_pct_reach_partial:>5.0%} {fs.net_missed_ev:>8.1f}"
