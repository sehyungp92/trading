...
def test_engine_instantiation():
    section("8. Engine Instantiation — All 4 engines")
    from research.backtests.stock.config_alcb import ALCBBacktestConfig
    from research.backtests.stock.config_iaric import IARICBacktestConfig
    from research.backtests.stock.engine.research_replay import ResearchReplayEngine
    ...
    e1 = ALCBDailyEngine(cfg_alcb, replay)
    check("ALCBDailyEngine instantiates", e1 is not None)
    check("ALCBDailyEngine has run()", hasattr(e1, "run"))

    # IARIC Daily
    from research.backtests.stock.engine.iaric_daily_engine import IARICDailyEngine
    e2 = IARICDailyEngine(cfg_iaric, replay)
    check("IARICDailyEngine instantiates", e2 is not None)
    check("IARICDailyEngine has run()", hasattr(e2, "run"))

    # ALCB Intraday (Tier 2)
    ...
    # IARIC Intraday (Tier 2)
    from research.backtests.stock.engine.iaric_engine import IARICIntradayEngine
    e4 = IARICIntradayEngine(cfg_iaric, replay)
    check("IARICIntradayEngine instantiates", e4 is not None)
    check("IARICIntradayEngine has run()", hasattr(e4, "run"))


# ── 9. Optimization Modules ─────────────────────────────────────────
def test_optimization():
    section("9. Optimization — ParamSpace + Grid")
    from research.backtests.stock.optimization.param_space import ParamRange, build_grid

    ranges = [
        ParamRange(name="max_positions", low=3, high=7, step=2, default=5),
        ParamRange(name="heat_cap_r", low=4.0, high=8.0, step=2.0, default=6.0),
    ]
    grid = build_grid(ranges)
    check("Grid has 3×3=9 combos", len(grid) == 9,
          f"got {len(grid)}")
    check("First combo has max_positions", "max_positions" in grid[0])
    check("First combo has heat_cap_r", "heat_cap_r" in grid[0])

    # Verify values
    mp_values = sorted(set(g["max_positions"] for g in grid))
    check("max_positions values [3,5,7]", mp_values == [3, 5, 7],
          f"got {mp_values}")
