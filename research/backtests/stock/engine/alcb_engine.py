...
class ALCBIntradayEngine:
        ...
        while current <= end:
            ...
            if ablation.use_regime_gate and regime_tier == "C":
                for sym in list(positions):
                    ...
                    equity += closed.pnl - closed.commission
                    del positions[sym]
                current += timedelta(days=1)
            ...
            for bar_idx in range(max_bars):
                for sym, bars in all_bars.items():
                    ...
                    if sym in positions:
                        ...
                        if exited:
                            equity += trades[-1].pnl - trades[-1].commission
                        continue  # done with this symbol/bar
                    ...
                    self._try_entry(
                        ...
                        prior_day, positions, artifact, regime_tier, equity,
                        settings, ablation, shadow, qe_phantom_slots,
                    )
        ...
        if ablation.use_partial_takes:
            ...
            if take:
                ...
                if partial_qty < pos.quantity:
                    if pos.direction == Direction.LONG:
                        partial_pnl = (bar.close - pos.entry_price) * partial_qty
                    else:
                        partial_pnl = (pos.entry_price - bar.close) * partial_qty
                    comm = _IBKR_COMM_PER_SHARE * partial_qty
    ...
    def _try_entry(
        ...
        """Check if bar triggers a new entry for symbol."""
        ...
        is_breakout, entry_type = is_momentum_breakout(
            ...
            use_rvol_filter=ablation.use_rvol_filter,
            use_cpr_filter=ablation.use_cpr_filter,
            use_pdh_breakout=ablation.use_prior_day_high_breakout,
        )
        if not is_breakout:
            return
        ...
        # Helper for shadow rejection recording
        ...
        dow_mult = settings.thursday_sizing_mult if current.weekday() == 3 else 1.0
        ...
        pos = _Position(
            ...
            opened_date=current,
            setup_tag=f"T1_{entry_type_str}",
        )
        positions[sym] = pos
    ...
    def _close_position(
        ...
        commission = comm_rate * total_qty * 2  # entry + exit
        commission += pos.realized_partial_commission

        slip_bps = self.broker.slippage_config.slip_bps_normal
        slippage = exit_price * (slip_bps / 10_000) * total_qty

        # R-multiple
        r_mult = 0.0
        if pos.risk_per_share > 0 and total_qty > 0:
            total_risk = pos.risk_per_share * total_qty
            r_mult = (pnl - commission - slippage) / total_risk
