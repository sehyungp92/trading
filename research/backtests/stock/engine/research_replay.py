...
def _build_date_index(df: pd.DataFrame) -> tuple[list[date], list[int]]:
    ...
    dates_arr = df.index.normalize().date
    date_to_last: dict[date, int] = {}
    for i, d in enumerate(dates_arr):
        date_to_last[d] = i  # last occurrence wins
    sorted_dates = sorted(date_to_last)
    last_ilocs = [date_to_last[d] for d in sorted_dates]
    return sorted_dates, last_ilocs
...
class ResearchReplayEngine:
    ...
    def __init__(
        ...
        # Selection caches — cache the RESULT of selection (small: ~20 candidates)
        # instead of the full snapshot (huge: 415 symbols × 250 bars each).
        # Keyed by trade_date; valid when settings don't change between experiments.
        ...
        self._5m_bar_dates: dict[str, list[date]] = {}  # per-bar dates for bisect
    ...
    def load_all_data(self) -> None:
        ...
        for sym in all_symbols:
            ...
            if path_5m.exists():
                df = load_bars(path_5m)
                self._intraday_5m_cache[sym] = df
                self._5m_didx[sym] = _build_date_index(df)
        ...
        logger.info(
            "ResearchReplayEngine loaded: %d daily, %d ref, %d 30m, %d 5m, %d trading dates",
            loaded, len(self._ref_cache), len(self._intraday_30m_cache),
            len(self._intraday_5m_cache), len(self._trading_dates),
        )
        if missing > 0:
    ...
    def _prebuild_bar_caches(self) -> None:
        ...
        # 5m bars -- pre-convert from DataFrames (eliminates itertuples overhead)
        td5 = timedelta(minutes=5)
        for sym, df in self._intraday_5m_cache.items():
            if df.empty:
                continue
            timestamps = [t.to_pydatetime() for t in df.index]
            opens = df["open"].values
            ...
            vols = df["volume"].values if "volume" in df.columns else np.zeros(len(df))
            n = len(df)
            bars_5: list[iaric_models.Bar] = [None] * n  # type: ignore[list-item]
            bar_dates_5: list[date] = [None] * n  # type: ignore[list-item]
            for i in range(n):
                ts_i = timestamps[i]
                bars_5[i] = iaric_models.Bar(
                    symbol=sym, start_time=ts_i, end_time=ts_i + td5,
                    open=float(opens[i]), high=float(highs[i]), low=float(lows[i]),
                    close=float(closes[i]), volume=float(vols[i]),
                )
                bar_dates_5[i] = ts_i.date()
            self._5m_bars[sym] = bars_5
            self._5m_bar_dates[sym] = bar_dates_5
    ...
    def clear_selection_cache(self) -> None:
        """Clear cached selection results. Call when strategy settings change."""
    ...
    def _build_alcb_research_symbol(
        ...
        """Build an ALCB ResearchSymbol for one stock on one date."""
        didx = self._daily_didx.get(sym)
        arrs = self._daily_arrs.get(sym)
        if didx is None or arrs is None:
            return None
        ...
        bars_30m = self._slice_30m_bars(sym, trade_date)
    ...
    def build_alcb_snapshot(
        ...
        """
        market = self._compute_market_research(trade_date)
        sectors = self._compute_sector_research(trade_date)
        ...
        for sym, _, _ in self._universe:
            if use_filter and self._skip_symbol_prefilter(sym, trade_date, min_price, min_adv_usd):
                continue
            rs = self._build_alcb_research_symbol(sym, trade_date)
            if rs is not None:
                symbols[sym] = rs
    ...
    def alcb_selection_for_date(
        ...
        Results are cached by date (valid when settings are unchanged
        across experiment runs, which is the case for ablation studies).
        """
        cached = self._alcb_selection_cache.get(trade_date)
        if cached is not None:
            return cached
        ...
        self._alcb_selection_cache[trade_date] = result
        return result
