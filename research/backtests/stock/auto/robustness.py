...
def walk_forward_check(
    ...
    """Run walk-forward optimization and check efficiency.

    Uses existing walk_forward_alcb/walk_forward_iaric (Tier 1 internally).

    Returns:
        (stable, efficiency) where stable means efficiency > 0.5
    """
    from research.backtests.stock.optimization.param_space import (
        ALCB_PARAM_SPACE,
        IARIC_PARAM_SPACE,
    )
    ...
    try:
        if strategy == "alcb":
            ...
            result = walk_forward_alcb(replay, ALCB_PARAM_SPACE)
        elif strategy == "iaric":
            from research.backtests.stock.optimization.walk_forward import walk_forward_iaric
            result = walk_forward_iaric(replay, IARIC_PARAM_SPACE)
        else:
            return True, 0.0

        efficiency = result.efficiency
        return efficiency > 0.5, efficiency

    except Exception:
        logger.warning("Walk-forward check failed for %s", strategy)
        return False, 0.0
...
def run_robustness(
    ...
    # Walk-forward (optional, expensive)
    if not skip_walk_forward and replay is not None and experiment.strategy != "portfolio":
        wf_stable, wf_eff = walk_forward_check(experiment.strategy, replay)
        report.walk_forward_stable = wf_stable
        report.walk_forward_efficiency = wf_eff
