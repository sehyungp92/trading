"""Run the optimal config from incremental optimization and save full diagnostics.

Usage:
    cd C:/Users/sehyu/Documents/Other/Projects/trading
    PYTHONUNBUFFERED=1 python -u -m research.backtests.stock.auto.output.save_optimal_diagnostic
"""
...
# Load optimal config from the incremental run
_RAW_OPTIMAL = json.loads(
    (OUTPUT_DIR / "optimal_config.json").read_text(encoding="utf-8")
)

# Support both old flat format and new nested {param_overrides, ablation} format
if "param_overrides" in _RAW_OPTIMAL and isinstance(_RAW_OPTIMAL["param_overrides"], dict):
    OPTIMAL_PARAM_OVERRIDES: dict = _RAW_OPTIMAL["param_overrides"]
    OPTIMAL_ABLATION: dict = _RAW_OPTIMAL.get("ablation", {})
else:
    # Legacy flat format — entire dict is param_overrides
    OPTIMAL_PARAM_OVERRIDES = _RAW_OPTIMAL
    OPTIMAL_ABLATION = {}
...
def build_config(param_overrides: dict, ablation: dict | None = None) -> ALCBBacktestConfig:
    ...
    if ablation:
        ...
        # Also convert list values back to tuples for tuple-typed fields
...
def main():
    print("=" * 70)
    print("ALCB Optimal Baseline - Full Diagnostic Report")
    print("=" * 70, flush=True)
    ...
    # Run baseline
    print("\n[2/4] Running baseline...", flush=True)
    t0 = time.time()
    bl_config = build_config(BASELINE_OVERRIDES)
    ...
    print(f"  Baseline done in {time.time() - t0:.1f}s — {len(bl_result.trades)} trades", flush=True)

    # Run optimal with shadow tracker
    print("\n[3/4] Running optimal config with shadow tracker...", flush=True)
    t0 = time.time()
    ...
    print(f"  Optimal done in {time.time() - t0:.1f}s — {len(opt_result.trades)} trades", flush=True)
    ...
    report_lines = [
        "=" * 70,
        "ALCB Incremental Optimal Baseline — Full Diagnostic Report",
        "=" * 70,
        ...
        "",
        "Baseline Config:",
        json.dumps(BASELINE_OVERRIDES, indent=2, sort_keys=True),
        "",
        "Optimal Config:",
        json.dumps(_RAW_OPTIMAL, indent=2, sort_keys=True, default=list),
        "",
        "Mutations Applied:",
        "  (From greedy optimization — see optimal_config.json)",
        "",
        "╔══════════════════════════════════════════════════════════════════════╗",
        "║  PERFORMANCE COMPARISON                                             ║",
        "╚══════════════════════════════════════════════════════════════════════╝",
        "",
        fmt_metrics(bl_metrics, bl_score, "BASELINE"),
        "",
        fmt_metrics(opt_metrics, opt_score, "OPTIMAL"),
        "",
        f"  {'─' * 50}",
        f"  IMPROVEMENT",
        f"  {'─' * 50}",
    ...
    out_path = OUTPUT_DIR / "optimal_baseline.txt"
    out_path.write_text("\n".join(report_lines) + "\n", encoding="utf-8")
