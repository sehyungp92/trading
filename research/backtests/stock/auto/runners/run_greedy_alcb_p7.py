"""Run P7 quality & robustness greedy optimization for ALCB T2.

Tests 20 candidates (symbol quality, entry timing, FR refinements) via forward selection.
Base = P6 optimal config. New knobs: ATR USD filter, selection score, RS percentile,
late entry score escalation, FR grace/below_entry refinements, risk reduction.

Usage:
    cd C:/Users/sehyu/Documents/Other/Projects/trading
    PYTHONUNBUFFERED=1 python -u -m research.backtests.stock.auto.output.run_greedy_alcb_p7
"""
...
from research.backtests.stock.auto.greedy_optimize import (
    ALCB_T2_P7_BASE_MUTATIONS,
    ALCB_T2_P7_CANDIDATES,
    run_greedy,
    save_result,
)
...
def main():
    print("=" * 60)
    print("ALCB T2 Phase 7 Quality & Robustness Greedy Optimization")
    print("=" * 60, flush=True)
    ...
    result = run_greedy(
        ...
        strategy="alcb",
        tier=2,
        base_mutations=ALCB_T2_P7_BASE_MUTATIONS,
        candidates=ALCB_T2_P7_CANDIDATES,
        max_workers=n_workers,
    )

    # Save greedy result JSON
    save_result(result, OUTPUT_DIR / "greedy_optimal_alcb_p7.json")

    # Save optimal_config.json
    optimal = resolve_config(result.final_mutations)
    config_path = OUTPUT_DIR / "optimal_config_alcb_p7.json"
