...
class AutoBacktestHarness:
    ...
    # Params that affect selection results -- must clear cache when sweeping these
    _SELECTION_PARAMS = frozenset({
        "min_adv_usd", "min_containment", "max_squeeze_metric",
        "breakout_tolerance_pct", "min_price", "max_price",
    })
        ...
        if is_ablation and abs(delta_pct) < 1e-10:
            ...
            if not skip_robustness and delta_pct >= 0.02:
                ...
                robustness = run_robustness(
                    ...
                    replay=self.replay,
                    skip_walk_forward=skip_robustness,
                )
    ...
    def _run_engine(
        ...
        if strategy == "alcb":
            if tier == 1:
                ...
                engine = ALCBDailyEngine(config, self.replay)
            else:
