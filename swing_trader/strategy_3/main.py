"""Swing Breakout v3.3-ETF — entry point that wires everything together."""
...
logger = logging.getLogger("swing_breakout")


async def main() -> None:
    """Wire up IB session, OMS, instruments, and start the Breakout engine."""
    ...
    unit_risk_dollars = RiskCalculator.compute_unit_risk_dollars(
        nav=equity, unit_risk_pct=0.005,  # 0.50% base risk per Breakout spec §14.1
    )
    ...
    logger.info("Swing Breakout v3.3-ETF running — press Ctrl+C to stop")
    ...
    logger.info("Swing Breakout v3.3-ETF shutdown complete")
