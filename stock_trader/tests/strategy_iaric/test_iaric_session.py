"""Session/bootstrap tests for the IARIC runtime."""
...
from strategy_iaric.models import RegimeSnapshot, WatchlistArtifact
import strategy_iaric.session as iaric_session
...
def _install_runtime_patches(monkeypatch, captured: dict, *, nav: float):
    ...
    monkeypatch.setattr(iaric_session, "IBSession", _FakeIBSession)
    monkeypatch.setattr(
        iaric_session,
        "ContractFactory",
    ...
    monkeypatch.setattr(iaric_session, "fetch_nav", lambda session, default_nav=100_000.0: asyncio.sleep(0, result=nav))
    monkeypatch.setattr(iaric_session, "InstrumentationManager", _FakeInstrumentationManager)
    monkeypatch.setattr(iaric_session, "InstrumentationKit", _FakeInstrumentationKit)

    async def _build_oms_service(**kwargs):
        captured.update(kwargs)
        return _FakeOMS()

    monkeypatch.setattr(iaric_session, "build_oms_service", _build_oms_service)
...
def test_bootstrap_runtime_uses_allocated_nav_in_combined_mode(monkeypatch):
    ...
    monkeypatch.setenv("STOCK_TRADER_CAPITAL_ALLOCATION_US_ORB_PCT", "25")
    monkeypatch.setenv("STOCK_TRADER_CAPITAL_ALLOCATION_ALCB_PCT", "25")

    services = asyncio.run(iaric_session.bootstrap_runtime("config", settings=StrategySettings()))
    ...
    assert captured["unit_risk_dollars"] == 125.0
    ...
    assert engine._portfolio.account_equity == 50_000.0

    asyncio.run(iaric_session.shutdown_runtime(services))


def test_bootstrap_runtime_uses_full_nav_in_solo_mode(monkeypatch):
    captured: dict = {}
    _install_runtime_patches(monkeypatch, captured, nav=100_000.0)
    monkeypatch.setenv("STOCK_TRADER_DEPLOY_MODE", "iaric")
    ...
    assert captured["unit_risk_dollars"] == 250.0
...
def test_fetch_nav_falls_back_in_dev(monkeypatch):
    ...
    nav = asyncio.run(iaric_session.fetch_nav(session, default_nav=12_345.0))

    assert nav == 12_345.0


def test_fetch_nav_raises_in_live_without_account_data(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="Refusing to size positions off a guessed NAV"):
        asyncio.run(iaric_session.fetch_nav(session))
...
def test_fetch_nav_reads_net_liquidation(monkeypatch):
    ...
    session = _FetchNavSession(
        accounts=["DU1"],
        summary=[_FakeSummaryItem("NetLiquidation", "USD", "175000.50")],
    )

    nav = asyncio.run(iaric_session.fetch_nav(session))

    assert nav == 175000.50
