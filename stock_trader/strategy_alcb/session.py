"""Thin runtime bootstrap for ALCB."""
...
from .config import STRATEGY_ID, STRATEGY_TYPE, StrategySettings
...
async def bootstrap_runtime(config_dir: str | Path, settings: StrategySettings | None = None) -> RuntimeServices:
    ...
    effective_nav = resolve_paper_nav(raw_nav, Path("data/strategy_alcb/paper_capital_state.json"))
    ...
    unit_risk = RiskCalculator.compute_unit_risk_dollars(capital_allocation.allocated_nav, cfg.base_risk_fraction)
    oms = await build_oms_service(
        adapter=adapter,
        strategy_id=STRATEGY_ID,
        unit_risk_dollars=unit_risk,
        daily_stop_R=cfg.daily_stop_r,
        heat_cap_R=cfg.heat_cap_r,
        portfolio_daily_stop_R=cfg.portfolio_daily_stop_r,
        db_pool=bootstrap.pool,
        portfolio_rules_config=None,
        get_current_equity=lambda: capital_allocation.allocated_nav,
    )
    await oms.start()
    ...
    instrumentation_kit = InstrumentationKit(instrumentation, strategy_type=STRATEGY_TYPE)
