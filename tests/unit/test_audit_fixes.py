...
class TestTACuratedArtifacts:
    ...
    HANDLERS_PATH = Path(
        ...
        "trading_assistant/orchestrator/handlers.py"
    )
    BUILDER_PATH = Path(
        "_references/trading_assistant_agent/packages/trading_assistant/src/"
        "trading_assistant/skills/build_daily_metrics.py"
    )
...
"""
    ...
    def _add_trading_assistant_src_to_path():
        src = Path("_references/trading_assistant_agent/packages/trading_assistant/src").resolve()
    ...
    def test_assistant_brain_routes_forwarded_oms_events(self):
        src = Path(
            "_references/trading_assistant_agent/packages/trading_assistant/src/"
            "trading_assistant/orchestrator/orchestrator_brain.py"
    ...
    def test_assistant_builder_and_prompt_include_oms_curated_files(self):
        ...
        prompt = Path(
            "_references/trading_assistant_agent/packages/trading_assistant/src/"
            "trading_assistant/analysis/prompt_assembler.py"
        ).read_text(encoding="utf-8")
        triage = Path(
            "_references/trading_assistant_agent/packages/trading_assistant/src/"
            "trading_assistant/analysis/daily_triage.py"
        ).read_text(encoding="utf-8")
