...
def test_portfolio_synergy_design_uses_all_five_momentum_strategies() -> None:
...
def test_portfolio_synergy_seed_overweights_clean_frequency_without_overweighting_helix() -> None:
    allocations = SEED_PORTFOLIO_CONFIG["strategy_allocations"]

    assert allocations["NQ_REGIME"]["priority"] == 0
    assert allocations["NQ_REGIME"]["base_risk_pct"] > allocations["AKC_Helix_v40"]["base_risk_pct"]
    assert allocations["VdubusNQ_v4"]["base_risk_pct"] > allocations["NQDTC_v2.1"]["base_risk_pct"]
    assert SEED_PORTFOLIO_CONFIG["portfolio_rules"]["max_total_positions"] == 5
